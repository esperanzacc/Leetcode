...
class Solution {
  ...
  var graph = Array(repeating: [(w: Int, node: Int)](), count: n)
    ...
    var heap = Heap<(w: Int, dest: Int)> { $0.w < $1.w }
    heap.insert((0, k - 1))
    ...
    while !heap.isEmpty {
        ...
        for _ in 0..<count {
            let (currW, curr) = heap.remove()!
            visited.insert(curr)
            
            for next in graph[curr] {
                let nextW = currW + next.w
                if !visited.contains(next.node) && nextW < distances[next.node]{
                    distances[next.node] = nextW
                    heap.insert((nextW, next.node))
                }
            }
        }
    }
